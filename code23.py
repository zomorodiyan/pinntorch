...
def main():
    ...
    for epochs, initial_weights in run_schedule:
        ...
        for epoch in range(epochs):
            ...
            # Initialize weights
            eps = 1.0
            num_loss_components = 6  # Number of PDE loss components
            num_bc_components = 13  # Number of BC loss components

            # Initialize cumulative losses
            cumulative_losses = {
                'pde': torch.zeros(num_loss_components),
                'sparse': torch.zeros(num_loss_components),
                'bc': torch.zeros(num_bc_components)
            }
            ...
            for interval in range(N_intervals):
                ...
                # Update temporal weights for next interval
                eps_ = 1.0
                temporal_weights['bc'] = torch.exp(-eps * cumulative_losses['bc'])

                aa = [weight.item()  for weight in temporal_weights['bc']]
                print('bc temporal weights :', aa)

                temporal_weights['pde'] = torch.exp(-eps * cumulative_losses['pde'])
                temporal_weights['sparse'] = torch.exp(-eps * cumulative_losses['sparse'])

                # Apply temporal weights
                weighted_bc_losses = [loss * temporal_weights['bc'][i] for i, loss in enumerate(bc_losses)]
                weighted_pde_losses = [loss * temporal_weights['pde'][i] for i, loss in enumerate(pde_losses)]
                weighted_sparse_losses = [loss * temporal_weights['sparse'][i] for i, loss in enumerate(sparse_losses)]

                # Update cumulative losses
                cumulative_losses['bc'] += torch.tensor([loss.item() for loss in bc_losses])
                cumulative_losses['pde'] += torch.tensor([loss.item() for loss in pde_losses])
                cumulative_losses['sparse'] += torch.tensor([loss.item() for loss in sparse_losses])

            sparse_total_loss = sum(weights['sparse'][i] * cumulative_losses['bc'][i] for i in range(len(sparse_losses)))
            pde_total_loss = sum(weights['pde'][i] * cumulative_losses['pde'][i] for i in range(len(pde_losses)))
            bc_total_loss = sum(weights['bc'][i] * cumulative_losses['bc'][i] for i in range(len(bc_losses)))
