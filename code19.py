...
def train_step(model, optimizer, pde_inputs, boundary_conditions,
    ...
    total_norm_before = torch.sqrt(sum(p.grad.norm() ** 2 \
    ...
    total_norm_after = torch.sqrt(sum(p.grad.norm() ** 2 \
      for p in model.parameters() if p.grad is not None))
...
# Step 1: Calculate the residuals
    ...
    num_intervals = 99
    interval_size = (t_pde.max() - t_pde.min()) / num_intervals
# Step 2: Calculate the losses and weights for each interval
    interval_losses = []
    temporal_weights = []

# Step 3: Define the initial weight as a tensor of ones
    initial_weight = torch.ones(6, dtype=torch.float32, device=t_pde.device, requires_grad=True)
    temporal_weights.append(initial_weight)

# Step 4: Calculate the interval losses and weights
    for i in range(num_intervals):
        interval_min = t_pde.min() + i * interval_size
        interval_max = interval_min + interval_size
        interval_mask = (t_pde >= interval_min) & (t_pde < interval_max)

        # Calculate the losses for the current interval
        continuity_loss = torch.mean(continuity_residual[interval_mask] ** 2)
        x_momentum_loss = torch.mean(x_momentum_residual[interval_mask] ** 2)
        y_momentum_loss = torch.mean(y_momentum_residual[interval_mask] ** 2)
        k_loss = torch.mean(k_residual[interval_mask] ** 2)
        omega_loss = torch.mean(omega_residual[interval_mask] ** 2)
        c_loss = torch.mean(c_residual[interval_mask] ** 2)

        # Create a tensor of current interval losses
        current_losses = torch.stack([continuity_loss, x_momentum_loss, y_momentum_loss, k_loss, omega_loss, c_loss])
        interval_losses.append(current_losses)

        # Calculate the weight for the current interval
        if i == 0:
            weight = initial_weight
        else:
            sum_losses = torch.sum(torch.stack(interval_losses[:i]), dim=0)
            weight = torch.exp(-sigma * sum_losses)

        temporal_weights.append(weight)

# Step 5: Convert temporal_weights to tensor
    temporal_weights = torch.stack(temporal_weights)

# Step 6: Apply temporal weights to the interval losses
    weighted_continuity_losses = torch.stack([interval_losses[i][0] for i in range(num_intervals)]) * temporal_weights[1:, 0]
    weighted_x_momentum_losses = torch.stack([interval_losses[i][1] for i in range(num_intervals)]) * temporal_weights[1:, 1]
    weighted_y_momentum_losses = torch.stack([interval_losses[i][2] for i in range(num_intervals)]) * temporal_weights[1:, 2]
    weighted_k_losses = torch.stack([interval_losses[i][3] for i in range(num_intervals)]) * temporal_weights[1:, 3]
    weighted_omega_losses = torch.stack([interval_losses[i][4] for i in range(num_intervals)]) * temporal_weights[1:, 4]
    weighted_c_losses = torch.stack([interval_losses[i][5] for i in range(num_intervals)]) * temporal_weights[1:, 5]

# Step 7: Combine the weighted losses into a single loss term
    pde_losses = [
        torch.sum(weighted_continuity_losses),
        torch.sum(weighted_x_momentum_losses),
        torch.sum(weighted_y_momentum_losses),
        torch.sum(weighted_k_losses),
        torch.sum(weighted_omega_losses),
        torch.sum(weighted_c_losses)
    ]
    ...
    for bc in boundary_conditions:
        ...
        for variable, condition in conditions.items():
            if condition['type'] == 'Dirichlet':
                ...
                if dir_deriv == 'y':
                    if variable == 'u':
                        deriv = torch.autograd.grad(u_pred, y_b,
                          grad_outputs=torch.ones_like(u_pred), create_graph=True)[0]
                    elif variable == 'v':
                        deriv = torch.autograd.grad(v_pred, y_b,
                          grad_outputs=torch.ones_like(v_pred), create_graph=True)[0]
                    elif variable == 'p':
                        deriv = torch.autograd.grad(p_pred, y_b,
                          grad_outputs=torch.ones_like(p_pred), create_graph=True)[0]
                    elif variable == 'k':
                        deriv = torch.autograd.grad(k_pred, y_b,
                          grad_outputs=torch.ones_like(k_pred), create_graph=True)[0]
                    elif variable == 'omega':
                        deriv = torch.autograd.grad(omega_pred, y_b,
                          grad_outputs=torch.ones_like(omega_pred), create_graph=True)[0]
