...
def loss(model, x, y, t, source_k, source_omega, boundary_conditions, initial_conditions):
    # PDE residuals
...
def train_step(model, optimizer, x, y, t, source_k, source_omega, boundary_conditions, initial_conditions):
    optimizer.zero_grad()
    total_loss = loss(model, x, y, t, source_k, source_omega, boundary_conditions, initial_conditions)
    total_loss.backward()
    optimizer.step()
...
def main():
    ...
    for epoch in range(1000):
        loss_value = train_step(model, optimizer, x, y, t, source_k, source_omega, boundary_conditions, initial_conditions)
        if epoch % 100 == 0:
