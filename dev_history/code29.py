...
all_normalized_weights = {
    'pde': torch.tensor([0.2, 0.1, 0.1, 1000.0, 0.01, 1e6], device=device),
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs_/c29_Jun28_01')
...
# Loop over batches and intervals
            for interval, batch_data in enumerate(data_loader):
                ...
                raw_losses['pde'] = [criterion(residual, torch.zeros_like(residual)).to(device)
                                     for residual in pde_residuals(model, *pde_inputs)]
                raw_losses['bc'] = [loss.to(device) for loss in bc_calc_loss(model, boundary_conditions, criterion)]
                raw_losses['sparse'] = [criterion(model(torch.cat(inputs, dim=1))[i], outputs[i].squeeze()).to(device) for i in range(6)]
...
# Initialize cumulative losses matrix
            cumulative_losses = {
                'pde': torch.zeros((num_intervals, num_components_pde), device=device),
                'sparse': torch.zeros((num_intervals, num_components_sparse), device=device),
                'bc': torch.zeros((num_intervals, num_components_bc), device=device)
            }
...
# Calculate temporal weights
            eps_ = 100
            temporal_weights = {
                key: torch.ones((num_intervals, all_raw_losses[key].shape[1]), device=device)
            ...
            for interval in range(1, num_intervals):
                for key in ['bc', 'pde', 'sparse']:
                    temporal_weights[key][interval] = torch.clamp(
                        torch.exp(-eps_ * torch.cumsum(all_raw_losses[key][:interval], dim=0)[-1]),
                        min=1e-2, max=None
                    )

# Calculate temporal weighted losses
            temporal_weighted_losses = {
                'pde': torch.zeros_like(all_raw_losses['pde']),
                'sparse': torch.zeros_like(all_raw_losses['sparse']),
                'bc': torch.zeros_like(all_raw_losses['bc'])
            }
            ...
            for interval in range(num_intervals):
                ...
                if interval > 0:
                    for key in ['bc', 'pde', 'sparse']:
                        cumulative_losses[key][interval] = cumulative_losses[key][interval - 1] + temporal_weighted_losses[key][interval]
...
# Compute sum of temporal weighted losses for each key
            ...
            ic_losses = [weights['ic'][i] * raw_ic_losses[i] for i in range(len(weights['ic']))]
            bc_losses = [weights['bc'][i] * sum_temporal_weighted_losses['bc'][i] for i in range(len(weights['bc']))]
            sparse_losses = [weights['sparse'][i] * sum_temporal_weighted_losses['sparse'][i] for i in range(len(weights['sparse']))]
            pde_losses = [weights['pde'][i] * sum_temporal_weighted_losses['pde'][i] for i in range(len(weights['pde']))]

            # normalize using all_normalized_weights
            normalized_losses['ic'] = [all_normalized_weights['ic'][i] * ic_losses[i] for i in range(len(all_normalized_weights['ic']))]
            normalized_losses['sparse'] = [all_normalized_weights['sparse'][i] * sparse_losses[i] for i in range(len(all_normalized_weights['sparse']))]
            normalized_losses['pde'] = [all_normalized_weights['pde'][i] * pde_losses[i] for i in range(len(all_normalized_weights['pde']))]
            normalized_losses['bc'] = [all_normalized_weights['bc'][i] * bc_losses[i] for i in range(len(all_normalized_weights['bc']))]
...
# Compute and store gradient norms
            if epoch % N_weight_update == 0 and epoch != 0:
                ...
                min_weight = 1.0
                gradient_norms = {
                    'pde': torch.zeros(num_components_pde, device=device),
                    'sparse': torch.zeros(num_components_sparse, device=device),
                    'bc': torch.zeros(num_components_bc, device=device)
                }
...
# Perform backward pass
                for key in ['bc', 'pde', 'sparse']:
                    for i in range(len(weights[key])):
                        ...
                        # Store gradient norm for each component (same for all components since we sum over intervals)
                        gradient_norms[key].fill_(grad_norm)
...
#--- log metrics (tensorboard) -----------------------------------------------
            if epoch % N_log_metrics == 0:
                log_metrics(writer, tot_epoch, epoch, total_loss, ic_total_loss,
                            ...
                            raw_ic_losses, sum_temporal_weighted_losses['bc'],
