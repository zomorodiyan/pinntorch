...
def update_weights(model, x, y, t, Re, theta, boundary_conditions, initial_conditions, sparse_data, weights):
    ...
    # Determine the maximum value of the input x
    max_input_value = x.max()
    # Set the clamping threshold to 10 times the maximum input value
    clamp_threshold = 10 * max_input_value
    ...
    for key, grads in gradients.items():
        ...
        for grad, weight in zip(grads, weights[key]):
            if weight != 0:
                ...
                # Clamp the weight if it exceeds the threshold
                if updated_weight > clamp_threshold:
                    updated_weight = clamp_threshold
    ...
    for epochs, initial_weights in run_schedule:
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                ...
                weights_str = {key: [f"{w.item():.0e}" for w in value.cpu().numpy()] for key, value in weights.items()}
                print("pde:", ", ".join(weights_str['pde']), "bc:", ", ".join(weights_str['bc']), "ic:", ", ".join(weights_str['ic']), "sparse:", ", ".join(weights_str['sparse']))

            scheduler.step()


        plot_fields(1.0, model, x_sparse, y_sparse, t_sparse, Re_sparse,
        theta_sparse, 'code12') # provide U_star to plot dimensional values
