...
all_normalized_weights = {
    'pde': torch.tensor([0.2, 0.1, 0.1, 1000.0, 0.01, 1e4], device=device),
    ...
    'ic': torch.tensor([0.2, 0.2, 0.2, 1000, 0.01, 1e4], device=device),
    'sparse': torch.tensor([0.2, 0.2, 0.2, 1000, 0.01, 1e4], device=device)
}
...
def log_metrics(writer, tot_epoch, epoch, total_loss, ic_total_loss, bc_total_loss, sparse_total_loss, pde_total_loss,
    ...
    for i, ic_loss in enumerate(ic_losses):
        writer.add_scalar(f'bc/{ic_names[i]}', ic_loss, epoch)
    ...
    if epoch % N_plot_tweight == 0 and epoch != 0:
        ...
        for interval in range(10):
            ...
            ax.scatter([x_pde[interval]]*6,
                       ...
                       color='blue', marker='^', label='bc' if interval == 0 else "")
            ax.scatter([x_sparse[interval]]*6,
                       temporal_weights['sparse'][interval].cpu().detach().numpy(),
                       color='red', marker='^', label='bc' if interval == 0 else "")
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs_/c30_Jun28_04')
...
#--- log metrics (tensorboard) -----------------------------------------------
            print("all_normalized_weights['ic']:", all_normalized_weights['ic'])
            print("raw_ic_losses:", raw_ic_losses)
            if epoch % N_log_metrics == 0:
                log_metrics(writer, tot_epoch, epoch, total_loss, ic_total_loss,
                            ...
                            [normalized_raw_losses['bc'][i].item() for i in range(6)],
