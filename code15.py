...
# ---------------------------- Tensorboard -----------------------------------
    ...
    writer.add_scalar('loss_pde', loss_pde, epoch)
    writer.add_scalar('loss_bc', loss_bc, epoch)
    writer.add_scalar('loss_ic', loss_ic, epoch)
    writer.add_scalar('loss_sparse', loss_sparse, epoch)

    # Log grouped boundary condition losses
    bc_names = [
    ...
    if epoch % 1000 == 0:
        time = 1.0
        fig = plot_fields(time, model, *inputs, f'code15_{epoch}') # provide U_star for dimensional plot
        writer.add_figure('Prediction Fields', fig, epoch)
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs/code15_r6') # TensorBoard
