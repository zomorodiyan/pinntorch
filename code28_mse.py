...
def update_weights(model, criterion, dataset, data_loader, weights, writer, epoch):
    ...
    for batch_data in data_loader:
        ...
        boundary_conditions = boundary_conditions.float().requires_grad_(True)
        pde_inputs = pde_inputs.float().requires_grad_(True)
...
def main():
    ...
    writer = SummaryWriter(log_dir='runs_/c28_Jun26_05_mse')
    ...
    for epochs, initial_weights in run_schedule:
        ...
        for epoch in range(epochs):
            ...
            if epoch % N_weight_update == 0 and epoch != 0:
                new_weights = update_weights(model, criterion, dataset, data_loader, weights, writer, epoch)
