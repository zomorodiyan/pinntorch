...
from utils import calculate_range_and_divergence, check_for_nan_and_inf, check_tensor_stats
from utils import formatted_print, plot_fields
from monitor import forward_hook, register_hooks
...
def update_weights(model, inputs, boundary_conditions, initial_conditions, sparse_data, weights):
    ...
    _, loss_components = loss(model, x, y, t, Re, theta, boundary_conditions, initial_conditions, sparse_data, weights)
    ...
    clamp_threshold = 2 * max_weight_value
...
def loss(model, x, y, t, Re, theta, boundary_conditions, initial_conditions, sparse_data, weights):
    ...
    N_ic = 100
    total_ic = initial_conditions[0].size(0)
    ic_indices = torch.randperm(total_ic)[:N_ic]
    initial_conditions_subset = [ic[ic_indices] for ic in initial_conditions]
    x_0_subset, y_0_subset, t_0_subset, Re_0_subset, theta_0_subset,\
      u_0_subset, v_0_subset, p_0_subset, k_0_subset, omega_0_subset,\
        c_0_subset = initial_conditions_subset
    u_0_pred, v_0_pred, p_0_pred, k_0_pred, omega_0_pred, c_0_pred = \
      model(torch.cat([x_0_subset, y_0_subset, t_0_subset, Re_0_subset,\
        theta_0_subset], dim=1))
    ic_losses = [
        torch.mean((u_0_pred - u_0_subset) ** 2),
        torch.mean((v_0_pred - v_0_subset) ** 2),
        torch.mean((p_0_pred - p_0_subset) ** 2),
        torch.mean((k_0_pred - k_0_subset) ** 2),
        torch.mean((omega_0_pred - omega_0_subset) ** 2),
        torch.mean((c_0_pred - c_0_subset) ** 2)
    ]
