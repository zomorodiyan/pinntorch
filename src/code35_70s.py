...
def generate_boundary_conditions(interval, num_samples):
    ...
    t_low, t_high = 70 + interval * (10 // N_intervals), 70 + (interval + 1) * (10 // N_intervals)
...
class CustomDataset(Dataset):
    ...
    def get_initial_condition_batch(self, batch_size):
        indices = torch.randperm(self.elements_per_snapshot)[:batch_size] + 13001 * 5 * 70
        return self.data[indices]

    def get_plotting_data(self, snapshot, simulation):
        start_idx = (snapshot - 1) * self.elements_per_snapshot + (simulation - 1) * self.elements_per_simulation + 13001 * 5 * 70
        end_idx = start_idx + self.elements_per_simulation
        return self.data[start_idx:end_idx]

    def get_data_from_interval(self, interval):
        start_snapshot = (interval + 70) * self.snapshots_per_interval
        end_snapshot = (interval + 71) * self.snapshots_per_interval
...
def plot_fields(x, y, u, v, p, k, omega, c, snapshot,
    ...
    with torch.no_grad():
        ...
        plt.colorbar()
        plt.title(f'Predicted $u$ at time {70+snapshot}s [m/s]')
        plt.tight_layout()

        plt.subplot(3, 2, 3)
        plt.tricontourf(triang, v.squeeze(), cmap='jet', levels=100)
        plt.colorbar()
        plt.title(f'Predicted $v$ at time {70+snapshot}s [m/s]')
        plt.tight_layout()

        plt.subplot(3, 2, 5)
        plt.tricontourf(triang, p.squeeze(), cmap='jet', levels=100)
        plt.colorbar()
        plt.title(f'Predicted $p$ at time {70+snapshot}s [pa]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $k$ at time {70+snapshot}s [\(m^2/s^2\)]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $\omega$ at time {70+snapshot}s [\(1/s\)]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $c$ at time {70+snapshot}s')
        plt.tight_layout()
