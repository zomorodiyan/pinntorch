...
def generate_boundary_conditions(interval, num_samples):
    ...
    t_low, t_high = 20 + interval * (10 // N_intervals), 20 + (interval + 1) * (10 // N_intervals)
...
all_normalized_weights = {
    'pde': torch.tensor([0.2, 0.1, 0.1, 1000.0, 0.001, 1e9], device=device),
...
class CustomDataset(Dataset):
    ...
    def get_initial_condition_batch(self, batch_size):
        indices = torch.randperm(self.elements_per_snapshot)[:batch_size] + 13001 * 5 * 20
        return self.data[indices]

    def get_plotting_data(self, snapshot, simulation):
        start_idx = (snapshot - 1) * self.elements_per_snapshot + (simulation - 1) * self.elements_per_simulation + 13001 * 5 * 20
        end_idx = start_idx + self.elements_per_simulation
        return self.data[start_idx:end_idx]

    def get_data_from_interval(self, interval):
        start_snapshot = (interval + 20) * self.snapshots_per_interval
        end_snapshot = (interval + 21) * self.snapshots_per_interval
    ...
    if epoch % N_plot_tweight == 0:
        ...
        for interval in range(10):
            ...
            ax.scatter([x_pde[interval]]*6, temporal_weights['pde'][interval].cpu().detach().numpy(),
                       color='green', marker='^', edgecolors='black', label='pde' if interval == 0 else "")
            # sparse markers
            ax.scatter([x_pde[interval]]*6, temporal_weights['sparse'][interval].cpu().detach().numpy(),
                       color='green', marker='^', edgecolors='black', label='sparse' if interval == 0 else "")
...
# --- actuals ----------------------------------------------------------------
        ...
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t11', U_star = 9.0)
        writer.add_figure('Actual t = 11s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t12', U_star = 9.0)
        writer.add_figure('Actual t = 12s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t13', U_star = 9.0)
        writer.add_figure('Actual t = 13s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t14', U_star = 9.0)
        writer.add_figure('Actual t = 14s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t15', U_star = 9.0)
        writer.add_figure('Actual t = 15s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t17', U_star = 9.0)
        writer.add_figure('Actual t = 17s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t18', U_star = 9.0)
        writer.add_figure('Actual t = 18s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t19', U_star = 9.0)
        writer.add_figure('Actual t = 19s Fields', fig, epoch)
        snapshot, simulation = 9,1
        ...
        x,y,t,Re,theta = plot_inputs
        u,v,p,k,omega,c = plot_outputs
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c27_actual_t20', U_star = 9.0)
        writer.add_figure('Actual t = 20s Fields', fig, epoch)
    ...
    if epoch % N_plot_residuals== 0:
        ...
        fig = plot_fields(x,y,continuity, x_mom, y_mom, k_tr, omega_tr,
          conv_diff, snapshot, simulation, f'c32_resi_t21_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Residuals t = 21s', fig, epoch)
    ...
    if epoch % N_plot_error == 0:
        ...
        fig = plot_fields(x,y,up,vp,pp,kp,op,cp, snapshot, simulation, f'c32_errors_t21', U_star = 9.0)
        writer.add_figure('Errors t = 21s Fields', fig, epoch)
...
# --- predictions ------------------------------------------------------------
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t21_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 21s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t22_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 22s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t23_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 23s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t24_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 24s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t25_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 25s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t26_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 26s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t27_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 27s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t28_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 28s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t29_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 29s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35m_t30_epoch{epoch}', U_star = 9.0)
    ...
    if epoch % N_save_model == 0 and epoch != 0:
        save_model(model, '../c35_21_30.pth')
...
def plot_fields(x, y, u, v, p, k, omega, c, snapshot,
    ...
    with torch.no_grad():
        ...
        plt.colorbar()
        plt.title(f'Predicted $u$ at time {20+snapshot}s [$m/s$]')
        plt.tight_layout()

        plt.subplot(3, 2, 3)
        plt.tricontourf(triang, v.squeeze(), cmap='jet', levels=100)
        plt.colorbar()
        plt.title(f'Predicted $v$ at time {20+snapshot}s [$m/s$]')
        plt.tight_layout()

        plt.subplot(3, 2, 5)
        plt.tricontourf(triang, p.squeeze(), cmap='jet', levels=100)
        plt.colorbar()
        plt.title(f'Predicted $p$ at time {20+snapshot}s [pa]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $k$ at time {20+snapshot}s [$m^2/s^2$]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $\omega$ at time {20+snapshot}s [$1/s$]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $c$ at time {20+snapshot}s')
        plt.tight_layout()
...
def main():
    ...
    model.load_state_dict(torch.load('../models/c35_01_10.pth'))
    ...
    writer = SummaryWriter(log_dir='../runs/c35_21_30')
