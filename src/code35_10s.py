...
def generate_boundary_conditions(interval, num_samples):
    ...
    t_low, t_high = 10 + interval * (10 // N_intervals), 10 + (interval + 1) * (10 // N_intervals)
...
class CustomDataset(Dataset):
    ...
    def get_initial_condition_batch(self, batch_size):
        indices = torch.randperm(self.elements_per_snapshot)[:batch_size] + 13001 * 5 * 10
        return self.data[indices]

    def get_plotting_data(self, snapshot, simulation):
        start_idx = (snapshot - 1) * self.elements_per_snapshot + (simulation - 1) * self.elements_per_simulation + 13001 * 5 * 10
        end_idx = start_idx + self.elements_per_simulation
        return self.data[start_idx:end_idx]

    def get_data_from_interval(self, interval):
        start_snapshot = (interval + 10) * self.snapshots_per_interval
        end_snapshot = (interval + 11) * self.snapshots_per_interval
...
# --- actuals ----------------------------------------------------------------
    ...
    if epoch % N_plot_residuals== 0:
        ...
        fig = plot_fields(x,y,continuity, x_mom, y_mom, k_tr, omega_tr,
          conv_diff, snapshot, simulation, f'c32_resi_t11_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Residuals t = 11s', fig, epoch)
    ...
    if epoch % N_plot_error == 0:
        ...
        fig = plot_fields(x,y,up,vp,pp,kp,op,cp, snapshot, simulation, f'c32_errors_t11', U_star = 9.0)
        writer.add_figure('Errors t = 11s Fields', fig, epoch)
...
# --- predictions ------------------------------------------------------------
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t11_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 11s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t12_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 12s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t13_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 13s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t14_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 14s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t15_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 15s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t16_epoch{epoch}', U_star = 9.0)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t17_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 17s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t18_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 18s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35_t19_epoch{epoch}', U_star = 9.0)
        writer.add_figure('Predicted t = 19s Fields', fig, epoch)
        ...
        x,y,t,Re,theta = plot_inputs
        u, v, p, k, omega, c = model(torch.cat([x, y, t, Re, theta], dim=1))
        fig = plot_fields(x,y,u,v,p,k,omega,c, snapshot, simulation, f'c35m_t20_epoch{epoch}', U_star = 9.0)
    ...
    if epoch % N_save_model == 0 and epoch != 0:
        save_model(model, '../c35_11_20.pth')
...
def plot_fields(x, y, u, v, p, k, omega, c, snapshot,
    ...
    with torch.no_grad():
        ...
        plt.colorbar()
        plt.title(f'Predicted $u$ at time {10+snapshot}s [$m/s$]')
        plt.tight_layout()

        plt.subplot(3, 2, 3)
        plt.tricontourf(triang, v.squeeze(), cmap='jet', levels=100)
        plt.colorbar()
        plt.title(f'Predicted $v$ at time {10+snapshot}s [$m/s$]')
        plt.tight_layout()

        plt.subplot(3, 2, 5)
        plt.tricontourf(triang, p.squeeze(), cmap='jet', levels=100)
        plt.colorbar()
        plt.title(f'Predicted $p$ at time {10+snapshot}s [pa]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $k$ at time {10+snapshot}s [$m^2/s^2$]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $\omega$ at time {10+snapshot}s [$1/s$]')
        plt.tight_layout()
        ...
        plt.colorbar()
        plt.title(f'Predicted $c$ at time {10+snapshot}s')
        plt.tight_layout()
...
def main():
    ...
    writer = SummaryWriter(log_dir='../runs/c35_11_20')
