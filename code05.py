...
def generate_initial_conditions(device):
    ...
    t_0 = torch.zeros_like(x_0).to(device)  # Initial time t=0
    ...
    c_0 = torch.zeros_like(x_0).to(device)  # Assuming zero concentration for initial conditions
...
def generate_sparse_data(device):
    ...
    u_sparse = torch.tensor(u_ref, dtype=torch.float32).view(-1, 1).to(device)
    v_sparse = torch.tensor(v_ref, dtype=torch.float32).view(-1, 1).to(device)
    p_sparse = torch.tensor(p_ref, dtype=torch.float32).view(-1, 1).to(device)
    k_sparse = torch.tensor(k_ref, dtype=torch.float32).view(-1, 1).to(device)
    omega_sparse = torch.tensor(omega_ref, dtype=torch.float32).view(-1, 1).to(device)
    c_sparse = torch.zeros_like(x_sparse).to(device)  # Assuming zero concentration for sparse data
