...
# pde with temporal weights--------------------------------------------------------
    ...
    num_intervals = 99
    interval_size = 99 / num_intervals
...
# Step 4: Calculate the interval losses and weights
    for i in range(num_intervals):
        interval_min = 1.0
        interval_max = 100.0
        mu_rho_l2 = mu/rho/L_star**2
        interval_mask = (t_pde/(Re_pde*mu_rho_l2) >= interval_min) & (t_pde/(Re_pde*mu_rho_l2) < interval_max)
...
def main():
    ...
    for epochs, initial_weights in run_schedule:
        ...
        for epoch in range(epochs):
            ...
            mu_rho_l2 = mu/rho/L_star**2
            t_pde = (mu_rho_l2 * Re_sparse[pde_indices].squeeze() +
                     99 * mu_rho_l2 * Re_sparse[pde_indices].squeeze() *
