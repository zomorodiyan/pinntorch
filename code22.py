...
# sparse ---------------------------------------------------------------------
    ...
    sparse_losses = [
        torch.mean(u_sparse_pred - u_sparse),
        torch.mean(v_sparse_pred - v_sparse),
        torch.mean(p_sparse_pred - p_sparse),
        torch.mean(k_sparse_pred - k_sparse),
        torch.mean(omega_sparse_pred - omega_sparse),
        torch.mean(c_sparse_pred - c_sparse),
    ]

# Step 9: Calculate the final sparse loss
...
def main():
    ...
    for epochs, initial_weights in run_schedule:
        ...
        for epoch in range(epochs):
            # Define the total length and the number of sections
            total_length = 13001 * 100
            sections = 100
            indices_per_section = 20
            N_pde = sections * indices_per_section

            # Generate the random indices with the given constraint
            pde_indices = []
            for i in range(sections):
                start_idx = i * 13001
                end_idx = (i + 1) * 13001
                section_indices = torch.randperm(13001)[:indices_per_section] + start_idx
                pde_indices.append(section_indices)
            # Concatenate all the indices to form the final pde_indices
            pde_indices = torch.cat(pde_indices)

            mu_rho_l2 = mu/rho/L_star**2
            pde_data_subset = [
                d[pde_indices].unsqueeze(1) if len(d.shape) == 1 else d[pde_indices]
                for d in [x_sparse, y_sparse, t_sparse, Re_sparse, theta_sparse]
            ]
